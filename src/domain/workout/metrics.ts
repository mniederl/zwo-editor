...
type PacedSegment = SteadySegment | RampSegment | IntervalSegment;

export const round = (x: number, roundTo: number): number => Math.round(x / roundTo) * roundTo;
...
export const getWorkoutLength = (bars: SegmentType[], durationType: DurationType): number =>
  bars.reduce((sum, b) => {
...
export const getWorkoutDistance = (bars: PacedSegment[]): number => bars.reduce((d, b) => d + b.length, 0) / 1000;

export const getWorkoutPace = (bars: SegmentType[], durationType: DurationType, paceUnitType: PaceUnitType): string => {
  const paced = bars.filter((b) => b.type !== "freeRide") as PacedSegment[];
...
export const getStressScore = (bars: SegmentType[], ftp: number): number => {
  const tss = bars.reduce((sum, b) => {
    ...
    if (b.type === "trapeze") {
      const avg = (b.startPower + b.endPower) / 2;
      const np = avg * ftp;
      return sum + b.time * np * avg;
    }
    if (b.type === "interval") {
      const npOn = b.onPower * ftp;
      const npOff = b.offPower * ftp;
      return sum + b.repeat * (b.onDuration * npOn * b.onPower + b.offDuration * npOff * b.offPower);
    }
    return sum;
  }, 0);
...
export const useWorkoutMetrics = (bars: SegmentType[], ftp: number, durationType: DurationType, paceUnitType: PaceUnitType) => {
  const paced = bars.filter((b) => b.type !== "freeRide") as PacedSegment[];
  const length = getWorkoutLength(paced, durationType);
  const distance = getWorkoutDistance(paced);
